...
(Carewell Pharmacy template) WITHOUT the diagonal watermark.
...
class Notes:
    ...
    def title_page(self):
        ...
        for line, col in [("Advances in", RED),
                          ("Drug Delivery", RED),
                          ("System", RED)]:
            p = d.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(line); r.bold = True; r.font.size = Pt(30)
        ...
        p = d.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p.add_run("M. Pharmacy \u2013 Pharmaceutical Technology \u2013 Semester I")
        ...
        if isinstance(it, tuple) and len(it) == 2 and isinstance(it[1], str):
            ...
            for seg in it:
                if isinstance(seg, str):
                    t, b, i = seg, False, False
                else:
                    t, b, i = (tuple(seg) + (False, False))[:3]
                r = p.add_run(t); r.bold = b; r.italic = i
                r.font.size = Pt(BODY_SIZE); r.font.color.rgb = BLACK; _set_font(r)
        else:
...
if __name__ == "__main__":
    ...
    out = os.path.join(root, "Advances in Drug Delivery System (MPT102T).docx")
    n.save(out)
    print("Saved", out, "| figures:", n.fig_no)
